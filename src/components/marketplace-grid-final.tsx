...
import { comparePricesForItem } from "@/services/priceComparison";
...
const MarketplaceGrid = ({
  ...
  const handleCompare = async (item: MarketplaceItem) => {
    setShowCompare(true);
    setCompareResult(null);
    const result = await comparePricesForItem(item);
            ...
            <div className="flex items-center gap-2">
              ...
              {voiceEnabled && (
                <Button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    speak(`${item.title}. ${'price' in item ? `$${(item as any).price}` : ''}`);
                  }}
                >
                  Listen
                </Button>
              ...
              <Button
                ...
                onClick={(e) => {
                  e.stopPropagation();
                  setExpertKeyword(item.category);
                  setShowExpert(true);
                }}
              >
                Talk to an Expert
  ...
  return (
    <>
      ...
      <PriceComparisonModal open={showCompare} onOpenChange={setShowCompare} result={compareResult} />
      <ExpertModal open={showExpert} onOpenChange={setShowExpert} contextKeyword={expertKeyword} />
      <ProductVariantModal open={showVariant} onOpenChange={setShowVariant} item={selectedItem} />
    </>
  );
};
